...
class DriverPendingOrderPage extends StatefulWidget {
  final String driverId;

  const DriverPendingOrderPage(this.driverId);
  @override
  _OrderPageState createState() => _OrderPageState();
}

class _OrderPageState extends State<DriverPendingOrderPage> {
            ...
            ? Container(
                child: StreamBuilder(
                    stream: Stream.fromFuture(
                        getDriverPenddingOrders(widget.driverId.toString())),
                    builder: (context, snapshot) {
                      ...
                      switch (snapshot.connectionState) {
                        ...
                        default:
                          ...
                          printRed(rs.body);
                          List jsonArray =
                              json.decode(rs.body)['AllDeliveryManOrders'];
                        if (json.decode(rs.body)['success'] == 0 ||
                              json.decode(rs.body)['success'] == -1 ||
                               json.decode(rs.body)['AllDeliveryManOrders'].length == 0) {
                            return const Center(
                              child: Text(
                                "لا يوجد طلبات",
                                style: TextStyle(
